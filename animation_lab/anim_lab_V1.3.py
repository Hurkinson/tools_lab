...
# Configuration des chemins et des ressources
title = "Test animation v.1.3"
project = "test2"
file_format = "jpg"
file_path = os.path.join(rf'C:\Users\vivie\Documents\My_py\toolbox\animation_lab", project)

def audit_folder(file_path):
    file_count = sum(1 for item in os.listdir(file_path) if os.path.isfile(os.path.join(file_path, item)))
    print(f"il y a {file_count} fichier(s) dans {file_path}")
    return file_count

file_count = audit_folder(file_path)
frame_count = (file_count - 1) // 2

# Pygame Configuration
pygame.init()
width, height, FPS = 400, 720, 60
screen = pygame.display.set_mode((width, height))
pygame.display.set_caption(title)
clock = pygame.time.Clock()

# Couleurs
WHITE, BLACK, RED, GREEN, BLUE, YELLOW = (255, 255, 255), (0, 0, 0), (255, 0, 0), (40, 200, 80), (0, 0, 255), (255, 255, 0)
...
class AnimationSprite(pygame.sprite.Sprite):
    def __init__(self, x, y, resolution='low'):
        super().__init__()
        self.load_frames(resolution)
        self.current_frame = 0
        self.image = self.frames[self.current_frame]
        self.rect = self.image.get_rect(topleft=(x, y))

    def load_frames(self, resolution):
        folder_name = 'pix_frame_' if resolution == 'low' else 'frame_'
        self.frames = [pygame.image.load(os.path.join(file_path, f"{folder_name}{i}.{file_format}")).convert_alpha() for i in range(frame_count)]

    def update(self, forward=True):
        self.current_frame = (self.current_frame + 1) % len(self.frames) if forward else (self.current_frame - 1) % len(self.frames)
        self.image = self.frames[self.current_frame]

# Groupe de sprites
anim_group = pygame.sprite.Group(AnimationSprite(-480, 0))
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_SPACE:
                paused = not paused
            if event.key in (pygame.K_RIGHT, pygame.K_LEFT) and paused:
                anim_group.update(event.key == pygame.K_RIGHT)

    screen.fill(BLACK)
    if not paused:
        anim_group.update()
    anim_group.draw(screen)
    draw_text(screen, "Paused" if paused else "Running", 30, width // 2, height // 2)
